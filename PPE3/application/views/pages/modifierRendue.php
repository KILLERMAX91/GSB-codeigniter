...
<!--  <title>formulaire RAPPORT_VISITE</title>-->
    ...
    <script language="javascript">
        ...
        //recupere select + values
         function getSelectValues(values){
            var chaine = "<select class='medocTab medicamentTab' name='medoc[]'>"; //plusieur select
            //recupere object Medicament
            $.each(json, function (key, medoc) {
                
                $.each(medoc, function (index, data) {
                    if(data.MED_DEPOTLEGAL===values){
                        chaine = chaine + "<option value='"+data.MED_DEPOTLEGAL+"' selected>"+data.MED_NOMCOMMERCIAL+"</option>";
                    }else{
                        chaine = chaine + "<option value='"+data.MED_DEPOTLEGAL+"'>"+data.MED_NOMCOMMERCIAL+"</option>";
                    }   
        ...
        function getTrValue(medoc, qua){
            var chaine = "<tr class='ligneTr'>";
            chaine = chaine + "<td>"+getSelectValues(medoc)+" <span class='btn btn-info medoc'><i class='icon-info-sign icon-white'></i></span></td>";
            chaine = chaine + '<td><input value="'+qua+'" class="qtMedocTab" style="position:relative;height:25px;" name="qtMedoc[]" type="number" id="replyNumber" min="0" data-bind="value:replyNumber" /></td>';
            chaine = chaine + '<td><span style="width:50px;" class="btn btn-inverse suppTr"> <i class="icon-trash icon-white"></i> </span></td>';
            chaine = chaine + "</tr>";
            return chaine;
        }
        $( document ).ready(function() {
            ...
            //affichage des medicament des utilisateur
            function lesMedicaments(){
            <?php  foreach ($lesMedocs as $row){ ?>
           
            if($( ".suppTr" ).length==0){
                $('#tableMedoc thead:last').after("<tbody>"+getTrValue(<?php echo "'".$row->MED_DEPOTLEGAL."','".$row->OFF_QTE."'"; ?>)+"</tbody>");
            }else{
                $('#tableMedoc tr:last').after(getTrValue(<?php echo "'".$row->MED_DEPOTLEGAL."','".$row->OFF_QTE."'"; ?>));
            }
            <?php } ?>
             $( ".medoc" ).click(function(e) {
                    var ind = $( ".medoc" ).index(this);
...
<div id="millieuBarre">
    ...
    <?php
        $attributForm = array('class' => 'form-horizontal', 'id' => 'FormNouveauRendue');
        if($rapport->saisieDefinitif==0){
            echo form_open('menu/modifierModifCompRendUpdate', $attributForm);
            
        }else{
            echo "<div class='form-horizontal'>";
        }
        echo form_fieldset('Modifier le compte Rendue');
        ?>
        <div class="control-group">
       		<label class="control-label" for="inputEmail">derniere modification: </label>
        	<div class="controls">
	        	 <?php $data = array(
	             
	              'class'   => 'input-large',
	              'value' => $rapport->DER_CONSUL,
        ...
        <div class="control-group">
            ...
            <?php $data = array(
              ...
              'style' => 'position:relative;height:25px;',
                ...
                echo form_hidden('numRapport', $rapport->RAP_NUM);
                echo form_hidden('numMatricule', $rapport->VIS_MATRICULE);
            echo form_input($data); ?> 
        ...
        <div class="control-group">
            ...
            <div class="controls">
               ...
               echo form_dropdown('PRACTITIEN', $lesPracticiens, $rapport->PRA_NUM, 'class="large" id="selectPracticiens"'); ?> <span id='infoPract' class="btn btn-info"><i class="icon-info-sign icon-white"></i> plus d'information</span>
            </div>
        </div>
        <div class="control-group">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <?php
                    echo form_dropdown("RAP_MOTIF", $tabMotif, $selected, "onClick=\"selectionne('AUT',this.value,'RAP_MOTIFAUTRE');\", class='zone' id='RAP_MOTIF'");
                ?>
                <script>
                    $( document ).ready(function() {
                    selectionne('AUT','<?php echo $selected ?>','RAP_MOTIFAUTRE');
                });
                </script>
       ...
       <div class="control-group">
           ...
           <script language="javascript">
            ...
            <?php  if($rapport->saisieDefinitif==0){ ?>
